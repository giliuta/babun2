...
export default function ClientCard({
  ...
  const initials =
    (clientName ?? "")
      ...
      .join("")
      .toUpperCase() || "?";
  const avatar = hasClient ? (
    <span className="w-[30px] h-[30px] rounded-full bg-gradient-to-br from-[#3E88F7] to-[#5E72E4] text-white text-[12px] font-bold flex items-center justify-center flex-shrink-0">
      {initials}
    </span>
  ) : undefined;
  ...
  return (
    <SectionCard>
      <AppointmentRow
        label="КЛИЕНТ"
        icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4" /><path d="M4 20c0-4 3.6-6 8-6s8 2 8 6" /></svg>}
        tileClass="bg-[var(--tile-blue)]"
        leading={avatar}
