    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end lg:items-center justify-center" onClick={onCancel}>
      ...
      >
        <div className="flex items-center gap-2 px-4 pt-4 pb-3 border-b border-slate-100">
          <h2 className="text-base font-semibold text-slate-900 flex-1">{title}</h2>
          <div
          ...
          {(["main", "hours"] as const).map((t) => (
            <button
              ...
              className={`px-3 py-2 text-sm font-medium border-b-2 transition-colors ${
                tab === t ? "border-violet-600 text-violet-700" : "border-transparent text-slate-500"
              }`}
            >
        ...
        <div className="overflow-y-auto flex-1 p-4">
          {tab === "main" ? (
            <div className="space-y-4">
              ...
              {type === "outsource" && (
                <div>
                  <label className="block text-xs text-slate-500 mb-1">Стоимость за выезд (€)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-2">Цвет на календаре</label>
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative w-9 h-5 rounded-full transition-colors ${isActive ? "bg-violet-600" : "bg-slate-300"}`}
                >
              ...
              <div>
                ...
                <div className="space-y-2">
                  {draftMembers.map((m) => (
                    <div key={m.id} className="bg-slate-50 rounded-lg p-2.5 space-y-2">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 text-[13px] font-medium text-slate-800">{nameOf(m.masterId)}</div>
                        <select
                          value={m.role}
                          onChange={(e) => updateMember(m.id, { role: e.target.value as BrigadeMember["role"] })}
                          className="text-[12px] border border-slate-300 rounded px-1.5 py-0.5 bg-white"
                        >
                      ...
                      {type === "internal" && (
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="text-[10px] text-slate-400">% от выручки</label>
                            <input
                              type="number"
                              min="0"
                              max="100"
                          ...
                          <div>
                            <label className="text-[10px] text-slate-400">Базовая ставка (€/мес)</label>
                            <input
                              type="number"
                              min="0"
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    type="button"
                    onClick={addMember}
                    className="px-3 py-1.5 bg-violet-50 text-violet-700 rounded-lg text-sm font-medium"
                  >
                    + Добавить
                  </button>
                </div>
              ...
              {WEEKDAY_KEYS.map((key) => {
                ...
                return (
                  <div key={key} className="flex items-center gap-2 py-1.5 border-b border-slate-50 last:border-0">
                    <div className="w-7 text-[12px] font-semibold text-slate-600">{WEEKDAY_NAMES[key]}</div>
                    <button
                      type="button"
                      onClick={() => updateDay(key, { isWorking: !day.isWorking })}
                      className={`relative w-8 h-4 rounded-full transition-colors flex-shrink-0 ${day.isWorking ? "bg-violet-600" : "bg-slate-300"}`}
                    >
                    ...
                    {day.isWorking ? (
                      <>
                        <select
                          ...
                          className="flex-1 text-[12px] border border-slate-300 rounded-lg px-1.5 py-1 bg-white"
                        >
                      ...
                      <span className="text-[12px] text-slate-400 ml-1">Выходной</span>
                    )}
                  </div>
                );
              })}
        ...
        <div className="px-4 py-3 border-t border-slate-100 flex gap-2">
          <button type="button" onClick={onCancel} className="flex-1 border border-slate-300 text-slate-700 rounded-lg px-4 py-2 text-sm">
            Отмена
          </button>
          <button type="button" onClick={submit} className="flex-1 bg-violet-600 text-white rounded-lg px-4 py-2 text-sm font-semibold">
            Сохранить
          </button>
        </div>
      </div>
...
function BrigadeCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <div className="text-sm font-semibold text-slate-900">{brigade.name}</div>
        ...
        <div className="flex items-center gap-1">
          <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${brigade.isActive ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
        ...
        <div className="mt-3 space-y-1.5">
          {brigadeMembers.map((m) => (
            <div key={m.id} className="flex items-center gap-2 bg-slate-50 rounded-lg px-2.5 py-1.5">
              <div className="w-5 h-5 rounded-full bg-violet-100 flex items-center justify-center text-violet-700 text-[10px] font-bold">
                {m.role === "lead" ? "Л" : "П"}
              </div>
              <div className="flex-1 text-[12px] text-slate-800">{m.masterId}</div>
              {brigade.type === "internal" && (
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-violet-700 font-semibold">{m.percentRate}%</span>
                  <span className="text-[11px] text-slate-400">{formatEURFromCents(m.baseMonthlySalaryCents)}/мес</span>
                </div>
              )}
            </div>
          ))}
      ...
      {brigadeMembers.length === 0 && (
        <div className="mt-2 text-[11px] text-slate-400">Нет участников</div>
      )}
    </div>
  );
}
        ...
        <div className="max-w-3xl mx-auto p-3 lg:p-4 space-y-3">
          ...
          {brigades.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 text-center">
              ...
              <div className="text-sm text-slate-500">Бригады ещё не загружены.</div>
              <div className="text-xs text-slate-400 mt-1">Перезагрузите страницу — сид-данные создадутся автоматически.</div>
            </div>
          ) : (
            brigades.map((b) => (
              <BrigadeCard
      ...
      {deleteWarning && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-5 max-w-sm w-full">
            <div className="text-sm font-semibold text-slate-900 mb-2">Удалить бригаду?</div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 border border-slate-300 text-slate-700 rounded-lg px-4 py-2 text-sm"
              >
                Отмена
              </button>
              <button
