...
| 6 | Единая `<ObjectForm />` для `/clients/new` + карточки | **DONE-partial (v562)** | New `<ObjectFormFields />` in `components/clients/`. Renders the shared field stack (type chips + label + address + note) so both consumers use one palette + one auto-fill rule. `/clients/new`'s `InlineLocationForm` migrated as the reference call site. `LocationEditor` (modal on `/clients/[id]`) still has its own fields — migration is a clean drop-in but bundled with the equipment list rework; left as a follow-up so this commit stays surgical. |
